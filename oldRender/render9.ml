...
let time_factor_constant (*the factor with which the event duration is multiplied during rendering*) = 480000.0
let dimension = 3
...
let rec draw_process_list (pl: proc list) = match pl with
           [] -> ()
        | ("F",(x,y),r)::t -> Graphics.close_graph ();exit 0
        (* if !semafor then read ();draw_process_list t    *)
...
let rec convert_species3D species_lst = match species_lst with
           ...
           | ("F",_)::t -> ("F",(0.,0.),0.)::(convert_species3D t)
...
let draw_time t =
  ...
  Graphics.draw_rect 200 159 180 25; 
  Graphics.fill_rect 200 159 180 25;
  Graphics.set_color ( Graphics.rgb 0 0 0 );
  Graphics.moveto 205 160;
...
let _ =
  ...
  let _ = print_string (file_name ^ "\n") in
  let ic:in_channel = open_in file_name in
  let lexbuf = Lexing.from_channel ic in
  let reac_list = Parser.reactions Lexer.token lexbuf in 
              (* print_string "."  *)
            (* write_reaction_list *)
		   draw_results (filter_reaction_list ["Cb";"Cx";"F";"Alb";"Alb1";"Alb2"] (dimension+1) reac_list)
