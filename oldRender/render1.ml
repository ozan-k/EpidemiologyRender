...
let rec erase_process_list (pl: proc list) = match pl with
           [] -> ()
        | (p,(x,y),r)::t -> Graphics.draw_circle (position_x + (int_of_float (x *. rc))) (position_y + (int_of_float (y *. rc))) rcr;
                        Graphics.fill_circle (position_x + (int_of_float (x *. rc))) (position_y + (int_of_float (y *. rc))) rcr;
...
let rec convert_species3D species_lst = match species_lst with 
                        [] -> []
           | (name,[x;y;z;r])::t -> (name, point ((float_of_string x) +. 0.5) ((float_of_string y) +. 0.5) ((float_of_string z) -. 0.0),float_of_string r)::(convert_species3D t) 
...
let _ =
  ...
  let reac_list = Parser.reactions Lexer.token lexbuf in
            (* write_reaction_list *) 
		   draw_results (filter_reaction_list ["Ab";"Al";"AlC";"A";"B";"C"] (dimension+1) reac_list)  
