...
let time_factor_constant (*the factor with which the event duration is multiplied during rendering*) = 6000.0
let dimension = 3
...
let rec convert_species3D species_lst = match species_lst with
                        [] -> []
           | (name,[x;y;z;r])::t -> (name, point (float_of_string y) (float_of_string x) ((float_of_string z) -. 0.0),compute_radius (float_of_string r))::(convert_species3D t)
...
let draw_time t =
  ...
  Graphics.draw_rect 235 134 250 50; 
  Graphics.fill_rect 235 134 250 50;
  Graphics.set_color ( Graphics.rgb 0 0 0 );
  Graphics.moveto 240 160;
  Graphics.draw_string t

let semafor = ref true

let read _  = 
      (* let _x = read_line () 
         in  *)
         tick 1.5;
         Sys.command "screencapture -R 60,630,464,261 -t png  ~/Desktop/s1.png";
        (* Sys.command "screencapture -R 60,630,464,261 -V 30  ~/Desktop/s1.avi &>/dev/null &";
         Sys.command "disown"; *)
         (* Sys.command "screencapture -R 75,640,432,243 -t png  ~/Desktop/s1.png"; *)
         semafor := false;
         tick 1.5;
...
let draw_event (time,left, right) =
                                   let t = float_of_string time in
                                   let f =      if t < 8. then 0.01
                                           else if t < 11. then 0.03
                                           else if t < 15. then 0.1
                                           else if t < 20. then 0.2
                                           else if t < 30. then 1.
                                           else if t < 35. then 3.
                                           else if t < 40. then 4.
                                           else if t < 45. then 6. 
                                           else if t < 50. then 12. 
                                           else if t < 55. then 16. 
                                           else                 18. in  
                                   (* draw_time time; *)
                                   (* if t < 60.0 && !semafor 
                                   then read ();          *)
                                   draw_time time;
                                   tick (t/. (f  *. time_factor_constant));
                                   Graphics.set_color ( Graphics.rgb 255 255 255 );
...
let draw_cube () =
   ...
   my_lineto  0.0 1.0 1.0 ;
   Graphics.set_color ( Graphics.rgb 0 160 0 );
   my_moveto  0.0 0.0 1.0 ;
   my_lineto  0.0 1.0 1.0 ;
   my_moveto  0.0 1.0 0.5 ;
   my_lineto  0.0 0.0 0.5 ;
   my_moveto  0.0 1.0 0.0 ;
   my_lineto  0.0 0.0 0.0 ;

   (* Back wall *)
   Graphics.set_color ( Graphics.rgb 0 160 0 );
   my_moveto  1.0 0.0 0.0 ;
   my_lineto  1.0 0.0 1.0 ;
   Graphics.set_color ( Graphics.rgb 160 0 0 );
   my_moveto  0.0 0.0 1.0 ;
   my_lineto  1.0 0.0 1.0 ;
   my_moveto  0.0 0.0 0.5 ;
   my_lineto  1.0 0.0 0.5 ;
   my_moveto  0.0 0.0 0.0 ;
   my_lineto  1.0 0.0 0.0 ;
   
   (* Base *)
   Graphics.set_color ( Graphics.rgb 0 0 160 );
   my_moveto  1.0 0.0 0.0 ;
   my_lineto  1.0 1.0 0.0 ;
   my_moveto  0.5 0.0 0.0 ;
   my_lineto  0.5 1.0 0.0 ;
   my_moveto  0.0 0.5 0.0 ;
   my_lineto  1.0 0.5 0.0 ;
   my_moveto  0.0 1.0 0.0 ;
   my_lineto  1.0 1.0 0.0 ;
   Graphics.set_color ( Graphics.rgb 0 0 0 );
   Graphics.moveto 175 161;
   Graphics.draw_string "Years: ";
   tick 5.0;
   ()

   (*

      my_lineto  0.0 1.0 0.0 ;
   my_lineto  0.0 0.0 0.0 ; (* black x = 0 *)
   Graphics.set_color ( Graphics.rgb 0 160 0 );
   my_moveto  0.0 0.0 0.0 ;
   my_lineto  0.0 0.0 1.0 ;
   my_moveto  1.0 0.0 1.0 ;
   my_lineto  1.0 0.0 0.0 ;
