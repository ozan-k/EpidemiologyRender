...
let paint_events event_list arr=
    ...
    let paint_individual species = match species with 
           ("I",[n]) -> paint_pair (map_point (0.0475 *.(float_of_string n))) red
         | ("S",[n]) -> paint_pair (map_point (0.0475 *.(float_of_string n))) green
         | ("R",[n]) -> paint_pair (map_point (0.0475 *.(float_of_string n))) blue
         |         _ -> ()
...
let _ =
  ...
  let reac_list = Parser.reactions Lexer.token lexbuf in
                 ...
                 let rec f_line n = if n >= (float_of_int max) 
                           then [] 
                           else (0.0475 *. n)::f_line (n+.1.) in
